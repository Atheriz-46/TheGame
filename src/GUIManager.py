...
class GUIManager(Tk):
    ...
    def keyboard(self):
        ...
        self.graphics.canvas.bind(
            "<Button-1>", lambda event: self.parent.rotateAntiClock()
        )
...
class ScoreBoard(Frame):
    ...
    def __init__(self, parent):
        ...
        self.lbl_leftScore = Label(
            text=self.parent.parent.state.players[0].points,
            fg="white",
            bg="black",
            master=self,
        )
        self.lbl_rightScore = Label(
            text=self.parent.parent.state.players[1].points,
            fg="white",
            bg="black",
            master=self,
        )
...
class Graphics(Frame):
    def __init__(self, parent):
        ...
        # self.scoreboard  = ScoreBoard(self)
        self.scale = (
            min(self.canvas.winfo_width(), self.canvas.winfo_height())
            / ARENA_X_BOUNDARY
        )
        self.draw()
    ...
    def circle(self, x, y, r, fill="blue", tags="bullet"):
        ...
        x, y, r = (
            x * self.scale + self.shift_x,
            y * self.scale + self.shift_y,
            r * self.scale,
        )
    ...
    def draw_gun(self, player, side):
        ...
        x1, y1 = (
            x + math.cos(orientation * 0.0174) * GUN_SIZE,
            y + math.sin(orientation * 0.0174) * GUN_SIZE,
        )
        x, y, x1, y1 = (
            x * self.scale + self.shift_x,
            y * self.scale + self.shift_y,
            x1 * self.scale + self.shift_x,
            y1 * self.scale + self.shift_y,
        )
        self.canvas.create_line(
            x, y, x1, y1, fill="black", width=GUN_WIDTH * self.scale, tags="shooter"
        )
